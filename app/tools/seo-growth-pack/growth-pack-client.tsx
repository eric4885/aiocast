...
import { ChevronDown, Loader2, Mic2, Plus, Sparkles, UploadCloud } from "lucide-react";
...
export function GrowthPackClient({
  ...
  const [audioNotesExpanded, setAudioNotesExpanded] = useState(false);
  ...
  const switchMode = (m: InputMode) => {
    ...
    if (m === "audio") {
      setAudioNotesExpanded(transcript.trim().length > 0);
    } else {
      setAudioNotesExpanded(false);
    }
  };
                  ...
                  {inputMode === "audio" && (
                    <>
                      ...
                      {!audioNotesExpanded ? (
                        <button
                          type="button"
                          onClick={() => setAudioNotesExpanded(true)}
                          className="mt-3 flex w-full items-center gap-2 rounded-lg border border-dashed border-border/80 bg-background/30 px-3 py-2.5 text-left text-sm text-muted-foreground transition-colors hover:border-primary/40 hover:bg-background/50 hover:text-foreground"
                        >
                          <Plus className="h-4 w-4 shrink-0 text-primary" aria-hidden />
                          {transcript.trim() ? (
                            <span>
                              Show notes added — <span className="text-foreground">tap to edit</span>
                            </span>
                          ) : (
                            <>
                        ...
                        <div className="mt-3 rounded-xl border border-border/70 bg-background/40 p-4">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-muted-foreground">
                                Optional — paste to skip auto-transcription. Audio alone is enough.
                              </p>
                            </div>
                            <button
                              type="button"
                              onClick={() => setAudioNotesExpanded(false)}
                          ...
                          <textarea
                            ...
                            placeholder="Paste show notes or transcript if you have them…"
